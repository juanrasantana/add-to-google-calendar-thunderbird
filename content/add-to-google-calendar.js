...
function checkICSattachment(e) {
	...
	for (let i in currentAttachments) {
		extension = currentAttachments[i].name.split('.').pop();
...
function startup() {
	...
	// We hide the view if it is not the messagepane (hide in threadContentArea)
	let threadContentArea = document.getElementById("threadContentArea");
	if (threadContentArea) {
		threadContentArea.addEventListener("contextmenu", function(e) {
			hideContextMenu();
		}, false);	
	}
